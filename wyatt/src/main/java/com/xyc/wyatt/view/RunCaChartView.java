 ...
 *
 */
public class RunCaChartView extends BaseChartView {

	@Override
	protected void drawBrokenChart() {
		// 绘制表头上的文字
		float maginLeft = left - textRedPaint.measureText("消耗卡路里")+10;
		mCanvas.drawText("消耗卡路里", maginLeft + left, BChartTop - 8, textRedPaint);
		...
		// 写最大值
		double max = getMaxMinCal()[0];
		double min = getMaxMinCal()[1];
		...
		if (records.size() > 0) {
			...
			path.moveTo(left, (int) (BChartTop+15 + ((float)max - records.get(0).getRunkaluli()) * heightScale));
			

			for (int i=1;i<records.size();i++) {
				RunRecord rr = records.get(i);
				cLeft =left + i*xper;
				cTop =  (int) (BChartTop+15 + ((float)max - rr.getRunkaluli()) * heightScale);
	...
	public RunCaChartView(Context context, AttributeSet attrs,
			int defStyle) {
	...
	public RunCaChartView(Context context) {
		super(context);
	}
