 ...
 *
 */
public class RunSpeedChartView extends BaseChartView {

	@Override
	protected void drawBrokenChart() {
		// 绘制表头上的文字
		float maginLeft = left - textRedPaint.measureText("运动速度") - 5;
		mCanvas.drawText("运动速度", maginLeft + left, BChartTop - 8, textRedPaint);
		...
		// 写最大值
		double max = getMaxMinSpeed()[0];
		double min = getMaxMinSpeed()[1];
		...
		if (records.size() > 0) {
			...
			path.moveTo(left, (int) (BChartTop+15 + ((float)max - records.get(0).getRunSpeed()) * heightScale));
			

			for (int i=1;i<records.size();i++) {
				RunRecord rr = records.get(i);
				cLeft =left + i*xper;
				cTop =  (int) (BChartTop+15 + ((float)max - rr.getRunSpeed()) * heightScale);
	...
	public RunSpeedChartView(Context context, AttributeSet attrs,
			int defStyle) {
	...
	public RunSpeedChartView(Context context) {
		super(context);
	}
